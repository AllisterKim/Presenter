...
5. 오공을 클릭해 메뉴를 열고 원하는 기능(프레젠테이션/그리기/타이머 등)을 선택하면 됩니다.
   자세한 사용법은 메뉴의 **"사용법"** 항목에서도 확인할 수 있습니다.
6. **숨기기**: 오공 메뉴의 "숨기기"를 누르면 화면에서만 사라지고, 트레이 아이콘은 그대로
   남습니다. 트레이 아이콘을 더블클릭하거나 우클릭 → "보이기"로 다시 꺼낼 수 있습니다.
7. **종료**: 오공 메뉴 또는 트레이 아이콘 우클릭의 "종료"를 누르면 한 번 더 종료 여부를
   확인한 뒤 프로세스까지 완전히 꺼집니다.
8. **제거**: Windows 설정 → 앱 → 설치된 앱에서 "오공 프레젠터"를 찾아 제거하거나,
